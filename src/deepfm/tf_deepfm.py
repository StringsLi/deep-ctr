...
class DeepFM(object):
    ...
    def _qcut(self, x, q, labels=None, precision=3):
        '''
    ...
    def load_data(self, train_dfn="adult.data", test_dfn="adult.test"):
        ...
        self.test_data = pd.read_csv(test_dfn, names=COLUMNS, skipinitialspace=True)
    ...
    def prepare_input_data(self, input_data):
        ...
        tmp = list(self.categorical_columns.keys())

        td = td[self.continuous_columns+tmp]
        ...
        for col in self.continuous_columns:
            ...
            bins, _ = self._qcut(td[col].values, bins_count, labels=False)
        ...
        X_mat = td.iloc[:, :].values
        Y_mat = Y.iloc[:, :].values
        X_codes_mat = X_codes.iloc[:, :].values
    ...
    def build_model(self, X, X_code):
        ...
        random_vector = np.random.rand(sample_size, k).astype(np.float32)
        inter_vector = tf.Variable(random_vector)

        V = tf.reshape(tf.nn.embedding_lookup(inter_vector, X_code), [sample_size, feature_code_size*k])

        second_order = np.zeros([sample_size, 1])
        for i in range(feature_code_size):
            for j in range(1, feature_code_size-i):
                former = V[:, i*k:(i+1)*k]
                later = V[:, (i+j)*k:(i+j+1)*k]
...
if __name__ == '__main__':
    deepfm = DeepFM()
    deepfm.load_data("E:\\spyder\\fm_python-master\\data\\adult.data", "E:\\spyder\\fm_python-master\\data\\adult.test")
    deepfm.train()
